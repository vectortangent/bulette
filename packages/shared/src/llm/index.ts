...
export function buildObrSystemPrompt(boardStateSummary: string): string {
  return [
    ...
    "If the user asks to generate an image (map, token art, prop, etc.), use OBR.assets.generateAndUploadImage with effect \"write\" and set safety.allowSceneUpload true. Args: prompt (required, detailed image description), name (display name), typeHint (\"MAP\", \"PROP\", \"CHARACTER\", \"MOUNT\", \"ATTACHMENT\", or \"NOTE\"), size (\"1024x1024\", \"1792x1024\", or \"1024x1792\"), addToScene (boolean, adds to scene immediately), position ({x,y} if addToScene). Example: {\"prompt\":\"A dark dungeon corridor with torchlight, top-down battle map style\",\"name\":\"Dungeon Corridor\",\"typeHint\":\"MAP\",\"size\":\"1792x1024\",\"addToScene\":true,\"position\":{\"x\":0,\"y\":0}}.",
    "If the user asks to create a token but does not mention characters/assets/library and no complete Owlbear item JSON is available, use OBR.assets.generateAndUploadImage with typeHint \"CHARACTER\" and a detailed prompt describing the token art (top-down perspective, circular token style).",
